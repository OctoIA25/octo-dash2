...
describe('paridade de entrypoints — atas de reunião', () => {
  for (const entry of ENTRYPOINTS) {
    const src = readFileSync(join(SERVER_DIR, entry), 'utf8');

    it(`${entry} importa e chama registerAtasRoutes`, () => {
      expect(src).toMatch(/import\s*\{[^}]*\bregisterAtasRoutes\b/);
      expect(src).toMatch(/registerAtasRoutes\s*\(\s*app\s*,\s*supabase\s*,\s*validateApiKey/);
    });

    it(`${entry} registra a rota antes do catch-all de 404`, () => {
      const registro = src.indexOf('registerAtasRoutes(app');
