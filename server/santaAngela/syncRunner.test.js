...
describe('createSantaAngelaRunner', () => {
  it('trigger() trava a guarda na hora e dispara o sync sem esperar terminar', async () => {
    ...
    const runner = createSantaAngelaRunner(syncService, { logger: silentLogger });
...
describe('startSantaAngelaScheduler', () => {
  it('agenda com o cron default e dispara o runner no tick', async () => {
    let registered = null;
    const cronImpl = { schedule: (expr, fn) => { registered = { expr, fn }; return { stop() {} }; } };
    const runner = { trigger: vi.fn() };
    await startSantaAngelaScheduler({}, { cronImpl, runner, processEnv: {} });
    expect(registered.expr).toBe('* * * * *');
    registered.fn();
    expect(runner.trigger).toHaveBeenCalledOnce();
  });

  it('SANTA_ANGELA_SYNC_CRON do env sobrescreve a expressão default', async () => {
    let registered = null;
    const cronImpl = { schedule: (expr, fn) => { registered = { expr, fn }; return { stop() {} }; } };
    const runner = { trigger: vi.fn() };
    await startSantaAngelaScheduler({}, { cronImpl, runner, processEnv: { SANTA_ANGELA_SYNC_CRON: '0 * * * *' } });
    expect(registered.expr).toBe('0 * * * *');
  });
});
