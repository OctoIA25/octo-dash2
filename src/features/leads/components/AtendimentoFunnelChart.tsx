...
/**
 * 🎯 FUNIL ATENDIMENTO
 * Etapas: Bolsão → Visita Realizada → Negociação → Proposta Criada → Proposta Enviada → Proposta Assinada
 * Começa após a Visita Agendada (que é a última do Pré-Atendimento)
 */
export const AtendimentoFunnelChart = ({ leads }: AtendimentoFunnelChartProps) => {
  const chartRef = useRef<HTMLDivElement>(null);
  const chartInstance = useRef<any>(null);
  const { currentTheme } = useTheme();

  // Calcular dados do funil - ATENDIMENTO (6 etapas: Bolsão até Proposta Assinada)
  const funnelData = useMemo(() => {
    const safeLeads = leads || [];

    // Definir etapas do Atendimento (começa em Bolsão, termina em Proposta Assinada)
    const etapasOrdem = [
      'Bolsão',
      'Visita Realizada',
      'Negociação',
    ...
    // Criar dataPoints com tamanhos harmônicos para 6 etapas
    const dataPoints = etapasOrdem.map((etapa, index) => {
      const quantidade = calcularEtapa(etapa);
      
      // 🎨 Valores FIXOS harmônicos para 6 etapas
      const valoresFixosHarmonicos = [100, 86, 72, 58, 44, 30];
      const valorVisualFixo = valoresFixosHarmonicos[index] || (100 - (index * 14));
    ...
    const metrics = {
      ...
      propostaEnviada,
      propostaAssinada,
      taxaConversaoGeral: bolsao > 0 ? (propostaAssinada / bolsao * 100) : 0,
      taxaVisitaRealizada: bolsao > 0 ? (visitaRealizada / bolsao * 100) : 0,
      ...
      taxaPropostaEnviada: propostaCriada > 0 ? (propostaEnviada / propostaCriada * 100) : 0,
      taxaPropostaAssinada: propostaEnviada > 0 ? (propostaAssinada / propostaEnviada * 100) : 0
    };
  ...
  useEffect(() => {
    ...
    const initializeChart = () => {
      ...
      // Estilização dos números - AGORA AZUL HARMÔNICO
      setTimeout(() => {
        ...
        labels?.forEach((label: Element, index: number) => {
          ...
          if (element.textContent && (element.textContent.includes('(') || /^\d+/.test(element.textContent.trim()) || element.textContent.includes('%'))) {
            const gradientColors = [
              ...
              'linear-gradient(135deg, #3B82F6 0%, #2563EB 100%)', // Azul Vibrante
              'linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%)', // Azul Forte
              'linear-gradient(135deg, #1D4ED8 0%, #1E40AF 100%)', // Azul Escuro
              'linear-gradient(135deg, #1E40AF 0%, #1E3A8A 100%)', // Azul Muito Escuro
              'linear-gradient(135deg, #1E3A8A 0%, #172554 100%)', // Azul Final
            ];
  ...
  const getFunnelColor = (etapa: string, index: number): string => {
    // 🎨 DEGRADÊ AZUL HARMÔNICO (6 Etapas - Atendimento)
    const colors = [
      '#60A5FA', // 1. Azul Claro
      '#3B82F6', // 2. Azul Vibrante
      '#2563EB', // 3. Azul Forte
      '#1D4ED8', // 4. Azul Escuro
      '#1E40AF', // 5. Azul Muito Escuro
      '#1E3A8A'  // 6. Azul Final
    ];
  ...
  return (
    <Card className="bg-bg-card/40 border-bg-secondary/40 shadow-xl shadow-black/20 h-full overflow-hidden transition-all duration-300 ease-in-out">
        <CardHeader className="pb-1">
          ...
          <p className="text-xs text-text-secondary mt-1">Bolsão → Proposta Assinada</p>
        </CardHeader>
          ...
          <div className="flex h-full transition-all duration-300 ease-in-out overflow-hidden">
            <div className="flex w-full h-full relative overflow-hidden">
              ...
              <div className="w-[65%] h-full flex justify-center items-center transition-all duration-300 ease-in-out overflow-hidden py-4">
                <div ref={chartRef} className="w-full h-full max-w-[450px] max-h-[600px] transition-all duration-300 ease-in-out" />
              </div>
            ...
            <div className="w-[35%] h-full relative py-4 -ml-6 flex items-center">
              ...
              {funnelData.dataPoints.map((point, index) => {
                ...
                const sectionHeight = 600 / totalSections;
                const topPosition = (index * sectionHeight) + (sectionHeight / 2) - 24;
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div 
                          className="text-sm font-bold neon-text-subtle leading-tight whitespace-nowrap"
                          style={{
